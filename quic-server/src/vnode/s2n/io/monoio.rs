...
use cfg_if::cfg_if;
use monoio::{net::udp::UdpSocket, runtime::Handle};
use s2n_quic_core::{
    endpoint::Endpoint,
...
use s2n_quic_platform::io::select::{self, Select};
use s2n_quic_platform::{buffer::default as buffer, features::gso, socket::default as socket};
...
impl Io {
    ...
    pub fn start<E: Endpoint<PathHandle = PathHandle>>(
        self,
        mut endpoint: E,
    ) -> io::Result<(monoio::task::JoinHandle<()>, SocketAddress)> {
        let Builder {
            handle,
            rx_socket,
            tx_socket,
...
impl<E: Endpoint<PathHandle = PathHandle>> Instance<E> {
    async fn event_loop(self) -> io::Result<()> {
        ...
        cfg_if! {
            if #[cfg(any(s2n_quic_platform_socket_msg, s2n_quic_platform_socket_mmsg))] {
                let rx_socket = monoio::io::unix::AsyncFd::new(rx_socket)?;
                let tx_socket = monoio::io::unix::AsyncFd::new(tx_socket)?;
            } else {
...
///
/// monoio only provides the AsyncFd interface for unix platforms so for
...
mod async_fd_shim {
    ...
    pub struct AsyncFd(monoio::net::UdpSocket);

    impl AsyncFd {
        pub fn new(socket: std::net::UdpSocket) -> io::Result<Self> {
            let socket = monoio::net::UdpSocket::from_std(socket)?;
            Ok(Self(socket))
        }
    ...
    pub struct TryIo<'a>(&'a monoio::net::UdpSocket);

    impl<'a> TryIo<'a> {
        pub fn try_io<R>(
            &mut self,
            f: impl FnOnce(&monoio::net::UdpSocket) -> io::Result<R>,
        ) -> Result<io::Result<R>, ()> {
            match f(self.0) {
                Ok(v) => Ok(Ok(v)),
